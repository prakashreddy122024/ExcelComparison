...
public class ExcelCompare {
    ...
    static String logFile = "ExcelUtility/data/Log.html";

    public static void main(String[] args) throws Exception {
        java.nio.file.Files.createDirectories(java.nio.file.Paths.get("data"));
        try (HtmlLogger htmlLogger = new HtmlLogger(logFile)) {
            ...
            htmlLogger.log("<b>" + now.format(formatter) + "</b>");
            htmlLogger.log("<hr>");
            // 1. Compare Workbook Sheets
            SheetComparator.compareSheets(excel1.workbook, excel2.workbook, file1, file2, htmlLogger);
            compareExcel(excel1.workbook, excel2.workbook, htmlLogger);
        }
    }


    public static void compareExcel(XSSFWorkbook beforeWb, XSSFWorkbook afterWb, HtmlLogger htmlLogger) throws Exception {
        ...
        for (int i = 0; i < beforeSheets; i++) {
            ...
            if (afterSheet == null) {
//                htmlLogger.log("Skipping comparison for missing sheet in AFTER: '" + sheetName + "'");
//                continue;
            }
            compareSheet(beforeSheet, afterSheet, htmlLogger);
        }
    }

    public static void compareSheet(XSSFSheet sheet1, XSSFSheet sheet2, HtmlLogger htmlLogger) throws Exception {
        RowColumnCountComparator.compareRowAndColumnCount(sheet1, sheet2, file1, file2, htmlLogger);
        HeaderComparator.compareHeaderCountAndNames(sheet1, sheet2, file1, file2, htmlLogger);
        RowColumnNameComparator.compareRowColumnNames(sheet1, sheet2, file1, file2, htmlLogger);
